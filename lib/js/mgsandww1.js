...
(function() {
  ...
  mgsAndWw1Site.config(function($routeProvider) {
    $routeProvider
      ...
      .when("/queryResult/:index", {
        templateUrl: "views/queryResult.htm",
  ...
  mgsAndWw1Site.controller("queryResult", function($scope, $routeParams) {
    ...
    if ((getObjects(dataset,"siteIndex",$routeParams.index)) && (getObjects(dataset,"siteIndex",$routeParams.index)).length) {
      $scope.hit = true;
      $scope.ourData = (getObjects(dataset,"siteIndex",$routeParams.index))[0]; // select the first object returned with the primary key
    } else {
